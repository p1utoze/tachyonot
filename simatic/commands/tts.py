...
async def tts_ms(text, language="en", gender="Male", output_file="output.mp3"):
...
@click.option("--all-voices", is_flag=True, help="List all available voices")
@click.argument('text', nargs=1)
def speech_gen(text, language, gender, output_file, locale, all_voices):
    if all_voices:
        subprocess.run(["edge-tts", "--list-voices"], check=True)
        return

    try:
        asyncio.run(tts_ms(text=text, language=language, gender=gender, output_file=output_file))
