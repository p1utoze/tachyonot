...
class LLamaCPPEmbedding:
    ...
    def __init__(self, embedding_model_path: str, chunk_size: int = 1024, chunk_overlap: int = 100):
        self.chunk_size = chunk_size
        self.chunk_overlap = chunk_overlap
        self.embedding_model = Llama(model_path=embedding_model_path, embedding=True, verbose=False)
        self.text_splitter = TextSplitter(max_token_size=self.chunk_size, end_sentence=True,
                                          preserve_formatting=True, language='english')
    ...
    def embed_documents(self, directory_path: str):
        document_paths = [os.path.join(directory_path, doc_name) for doc_name in os.listdir(directory_path)]
        embed_text = []
